...
bool Map::insert(const KeyType& key, const ValueType& value) {
  ...
  // capacity and is full).
    ...
    if (m_numPairs == DEFAULT_MAX_ITEMS) //Checks if max amount is reached
        return false;
...
bool Map::insertOrUpdate(const KeyType& key, const ValueType& value) {
  ...
  // capacity and is full).
    ...
    if (m_numPairs != DEFAULT_MAX_ITEMS) { //Checks if max amount is reached. If isn't add new pair.
        m_pairs[m_numPairs].key = key;
        m_pairs[m_numPairs].value = value;
        m_numPairs++;
        return true;
...
void Map::swap(Map& other) {
  // Exchange the contents of this map with the other one.
    ...
    for (int k = 0; k < m_numPairs; k++) {
        other.m_pairs[k].key = m_pairs[k].key;
        other.m_pairs[k].value = m_pairs[k].value;
    }
    
    m_numPairs = temp.m_numPairs;
    for (int k = 0; k < m_numPairs; k++) {
        m_pairs[k].key = temp.m_pairs[k].key;
        m_pairs[k].value = temp.m_pairs[k].value;
    }
}
