...
{
    //TODO [DONE]: You implement this.  Using the same criteria as compareStudent,
...
{
    // TODO [DONE]:  Using the insertion sort algorithm (pp. 311-313), sort s
    ...
    for(int k = 1; k < s.size(); k++) { //Starts at k = 1 because k = 0 would do nothing anyway
        int index = k;
        while (index > 0 && !(comp(s[index-1],s[index]))) {
...
int main()
{
      ...
      // destruction time.
    {
     vector<Student> auxStudents(students);
      ...
      // TODO [DONE]:  Create a vector of Student pointers, and set each pointer
